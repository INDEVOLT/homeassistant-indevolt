<!--
原因：英文 CHANGE.md 已形成完整版本记录，但中文审核者和用户无法快速确认同一版本边界。
目标：提供与英文版一一对应、术语统一且可独立阅读的简体中文变更日志。
实现：采用 CHANGE.zh-CN.md 的本地化文件名，以 zh-CN 语言标记明确简体中文；
版本顺序、章节结构、数值和限制与英文版保持一致，只保留功能变化、兼容边界和
用户可见的文档变化；1.2 先说明原有 2400 W 输入上限造成的问题，再列出文档交付。
影响：中文用户可直接审阅 1.0、1.1 和 1.2 的变化，不改变英文版、README 或集成行为。
边界：不增加部署或版本切换操作指南、内部函数、测试命令、发布日期或实机输出结论。
验证：已逐项核对两个文件的三个版本、功率范围、步长和 BK 边界。
风险：中英文文件后续可能发生内容漂移，因此后续维护时必须同时修改并交叉核对。
-->
